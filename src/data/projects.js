...
We introduce a clean-slate, forward-only evaluation protocol and benchmark that precludes retrospective contamination. Both training and test sets are restricted to forecasting questions posted after a model's knowledge cutoff. We release versioned, frozen snapshots of prediction-market questions along with time-stamped web search snapshots captured at scrape time—after the model's cutoff but before resolution. Crucially, these snapshots enable rigorous forward-only backtesting, allowing researchers to evaluate various forecasting strategies with replication even after markets resolve. Existing forecasting benchmarks and leaderboards do not support this kind of systematic backtesting and focus primarily on model comparisons. In contrast, our dataset allows instant replication without contamination and supports evaluation not only of different AI models but also of strategies such as prompting and information retrieval, which are central to advancing the forecasting capabilities of LLMs.`,
    },
    {
        id: "uplift",
        topic: ["research"],
